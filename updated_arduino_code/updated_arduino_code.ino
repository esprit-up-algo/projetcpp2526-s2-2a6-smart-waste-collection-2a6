...
// Lid travel and bin geometry. D2 rests at 180 deg, rotates to 90 deg
// to open. D6 is mirrored in setLid (rests at 0 deg, also opens to 90).
// Both servos over-close by LID_CLOSE_OVERSHOOT_DEG past their rest
// position to press the lid down firmly:
//   D2 -> 180 + 15 = 195 deg equivalent  (writeMicroseconds past 2400us)
//   D6 -> 0   - 15 = -15 deg equivalent  (writeMicroseconds below 544us)
// Both writes need writeMicroseconds with an extended attach range
// since Servo.write() clamps at 0..180.
const int LID_OPEN_DEG            = 90;
const int LID_CLOSE_DEG           = 180;
const int LID_CLOSE_OVERSHOOT_DEG = 15;  // applied to both MG90S at closed
...
// ----- Lid + HC-SR04 helpers --------------------------------------------
// D6 (mg1) is mirrored across the servo midpoint so its rest position is
// the inverse of D2's, but it still travels exactly 90 deg to meet D2 at
// 90 deg when the lid is open.
//
// At the closed position, BOTH servos over-rotate by LID_CLOSE_OVERSHOOT_DEG
// past their rest angle (in the opposite direction of opening) to seat
// the lid firmly. That's past 180 deg for D2 and below 0 deg for D6 -
// both require writeMicroseconds with extended attach ranges.
//
//   setLid(180) -> mg1 ~ -15deg (us~390),  mg2 ~ 195deg (us~2554)  closed
//   setLid(90)  -> mg1 = 90,               mg2 = 90                 open
//
// Default Servo lib map: 0deg = 544us, 180deg = 2400us (~10.3us per deg).
void setLid(int deg) {
  if (deg == LID_CLOSE_DEG) {
    int mg1Deg = -LID_CLOSE_OVERSHOOT_DEG;                     // -15
    int mg1Us  = 544 + (mg1Deg * (2400 - 544)) / 180;          // ~390us
    mg1.writeMicroseconds(mg1Us);

    int mg2Deg = LID_CLOSE_DEG + LID_CLOSE_OVERSHOOT_DEG;      // 195
    int mg2Us  = 544 + (mg2Deg * (2400 - 544)) / 180;          // ~2554us
    mg2.writeMicroseconds(mg2Us);
  } else {
    mg1.write(180 - deg);   // D6: mirrored
    mg2.write(deg);          // D2: direct
  }
}
...
void setup() {
  ...
  // Both MG90S need extended pulse ranges so writeMicroseconds() can
  // push them ~15 deg past their normal 0deg / 180deg endpoints for
  // the over-close behaviour in setLid(LID_CLOSE_DEG).
  mg1.attach(mg1Pin, 350, 2500);  // D6: pulse can drop below 544us  (~-15deg)
  mg2.attach(mg2Pin, 400, 2700);  // D2: pulse can rise above 2400us (~+195deg)
  sg.attach(sgPin);
