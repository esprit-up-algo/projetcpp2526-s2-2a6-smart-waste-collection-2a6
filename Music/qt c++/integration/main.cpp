...
int main(int argc, char *argv[])
{
    ...
    if (c && c->createConnect()) {
        const QString statusMessage = "Etat connexion BD: REUSSIE";
        qInfo().noquote() << statusMessage;
        out << statusMessage << Qt::endl;
        QMessageBox::information(nullptr, "Connexion base", statusMessage);
    } else {
        const QString errorMessage = c ? c->lastError() : QStringLiteral("Instance Connection indisponible.");
        const QString statusMessage = "Etat connexion BD: ECHEC - " + errorMessage;
        qCritical().noquote() << statusMessage;
        err << statusMessage << Qt::endl;
        QMessageBox::critical(nullptr,
                              "Connexion base",
                              "Echec de connexion a la base.\n" + errorMessage +
                                  "\nVerifiez le DSN ODBC 'Source_Projet2A', l'utilisateur et le mot de passe.");
    }
