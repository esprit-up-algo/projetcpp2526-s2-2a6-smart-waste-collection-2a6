...
QString moduleOverview(const QString &module)
{
    if (module == "Client") {
...
QString buildStepGuide(const QString &module)
{
    if (module == "Client") {
...
QString buildActionGuide(const QString &module, const QString &action)
{
    if (action == "modifier") {
...
ProviderConfig resolveProviderConfig()
{
    ...
    const QString openAiKey = env.value("OPENAI_API_KEY").trimmed();
    const QString deepSeekKey = env.value("DEEPSEEK_API_KEY").trimmed();
    const QString geminiKey = env.value("GEMINI_API_KEY").trimmed();
    const QString pref = env.value("WASTEGUARD_AI_PROVIDER").toLower().trimmed();

    const auto pickOpenAI = [&]() {
        config.apiKey = openAiKey;
        config.baseUrl = env.value("OPENAI_BASE_URL").trimmed();
        if (config.baseUrl.isEmpty()) config.baseUrl = "https://api.openai.com/v1";
        config.model = env.value("OPENAI_MODEL").trimmed();
        if (config.model.isEmpty()) config.model = "gpt-4o-mini";
        config.valid = !config.apiKey.isEmpty();
    };

    const auto pickDeepSeek = [&]() {
        config.apiKey = deepSeekKey;
        config.baseUrl = env.value("DEEPSEEK_BASE_URL").trimmed();
        if (config.baseUrl.isEmpty()) config.baseUrl = "https://api.deepseek.com/v1";
        config.model = env.value("DEEPSEEK_MODEL").trimmed();
        if (config.model.isEmpty()) config.model = "deepseek-chat";
        config.valid = !config.apiKey.isEmpty();
    };

    const auto pickGemini = [&]() {
        config.apiKey = geminiKey;
        config.baseUrl = env.value("GEMINI_BASE_URL").trimmed();
        if (config.baseUrl.isEmpty()) {
            config.baseUrl = "https://generativelanguage.googleapis.com/v1beta/openai";
        }
        config.model = env.value("GEMINI_MODEL").trimmed();
        if (config.model.isEmpty()) config.model = "gemini-2.0-flash";
        config.valid = !config.apiKey.isEmpty();
    };

    if (pref == "openai") pickOpenAI();
    else if (pref == "deepseek") pickDeepSeek();
    else if (pref == "gemini") pickGemini();
    else {
        if (!openAiKey.isEmpty()) pickOpenAI();
        else if (!deepSeekKey.isEmpty()) pickDeepSeek();
        else if (!geminiKey.isEmpty()) pickGemini();
    }

    return config;
}

QString queryCloudAssistant(const QString &userMessage, const QString &module)
{
    const ProviderConfig provider = resolveProviderConfig();
    if (!provider.valid) return QString();
    ...
    const QString systemPrompt =
        "Tu es Labib, assistant expert du projet WasteGuard: Smart Application for Waste Collection and Recycling Management. "
        "Reponds en francais naturel, precis et actionnable. "
        "Quand on demande une procedure (ex: ajouter client/intervention), donne des etapes numerotees. "
        "Tu dois te concentrer sur les modules: Clients, Employes, Produits, Maintenance, Commandes, Stock. "
        "Si module courant fourni, priorise-le. Module courant: " + module + ".";

    QJsonObject payload;
    payload.insert("model", provider.model);
    payload.insert("temperature", 0.25);
    ...
    req.setRawHeader("Authorization", QString("Bearer %1").arg(provider.apiKey).toUtf8());
...
void LabibAssistant::setupUI()
{
    ...
    m_typingLabel = new QLabel("Labib écrit...");
    ...
    connect(m_typingTimer, &QTimer::timeout, this, [this]() {
        ...
        m_typingLabel->setText(QString("Labib écrit%1").arg(QString(m_typingStep, '.')));
    });
...
void LabibAssistant::refreshSmartStats()
{
    ...
    m_statsLabel->setText(QString(
        "<b>%1</b> clients enregistrés &nbsp;&nbsp; • &nbsp;&nbsp; <b>%2</b> produits &nbsp;&nbsp; • &nbsp;&nbsp; "
...
void LabibAssistant::startTypingIndicator()
{
    ...
    m_typingStep = 0;
    m_typingLabel->setText("Labib écrit...");
    m_typingLabel->show();
    m_typingTimer->start();
    qApp->processEvents();
}
...
QString LabibAssistant::generateAiResponse(const QString &userMessage)
{
    ...
    QString mentionedModule = canonicalModuleName(normalized);
    if (mentionedModule.isEmpty()) {
        mentionedModule = m_selectedModule;
    }

    const bool asksHowTo = containsAny(normalized, {
        "comment", "etape", "etapes", "procedure", "guide", "quoi faire", "how to"
    });
    const bool asksAdd = containsAny(normalized, {
        "ajouter", "add", "creer", "creation", "insert"
    });
    const bool asksImport = containsAny(normalized, {
        "import", "fichier", "pdf", "csv", "excel", "automatique"
    });
    ...
    const bool asksProject = containsAny(normalized, {
        "smart waste", "wasteguard", "recyclage", "dechet", "projet", "application"
    });
    QString actionIntent = "ajouter";
    if (containsAny(normalized, {"modifier", "modif", "editer", "update", "mettre a jour"})) {
        actionIntent = "modifier";
    } else if (containsAny(normalized, {"supprimer", "effacer", "delete", "retirer"})) {
        actionIntent = "supprimer";
    } else if (containsAny(normalized, {"analyser", "analyse", "statistique", "rapport", "resumer"})) {
        actionIntent = "analyser";
    }

    if (asksHowTo && (asksAdd || !mentionedModule.isEmpty())) {
        return buildActionGuide(mentionedModule, actionIntent);
    }

    if (asksImport && normalized.contains("pdf")) {
        return "Oui, tu peux importer un PDF. Labib tente d'extraire automatiquement les informations utiles du document puis propose l'insertion en base.<br><br>"
               "Flux conseille:<br>"
               "1) Choisir le module (Client ou Maintenance).<br>"
               "2) Cliquer <b>Importer Fichier</b> puis selectionner le PDF.<br>"
               "3) Verifier l'aperçu des champs detectes.<br>"
               "4) Cliquer <b>Traiter Donnees</b> pour ajout automatique.";
    }

    if (asksProject) {
        return "WasteGuard est une application intelligente de collecte et recyclage des dechets: gestion clients, employes, interventions de maintenance, commandes, produits et stock, avec IA/OCR/PDF pour automatiser les operations.";
    }

    if (!mentionedModule.isEmpty() && (asksHowTo || normalized.contains("explique") || normalized.contains("detail"))) {
        return moduleOverview(mentionedModule) + "<br><br>" + buildActionGuide(mentionedModule, actionIntent);
    }
    ...
    const QString cloudAnswer = queryCloudAssistant(userMessage, mentionedModule);
    if (!cloudAnswer.isEmpty()) {
        return cloudAnswer;
    }

    if (containsAny(normalized, {"aide", "help", "assistant"})) {
        return "Je peux repondre en langage naturel sur tout le projet WasteGuard et te guider pas a pas. Exemples:<br>"
               "• Comment ajouter un client ?<br>"
               "• Donne-moi le workflow de maintenance<br>"
               "• Importe ce PDF et ajoute le client";
    }

    return "Je peux t'aider sur ce besoin. Precise juste le module cible (Client, Employe, Produit, Maintenance, Commande ou Stock) et je te donne un plan d'action detaille.";
}
