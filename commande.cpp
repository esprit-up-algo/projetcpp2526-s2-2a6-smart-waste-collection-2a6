...
Commande::Commande(int id, int idClient, int qte, const QString &priorite,
    ...
    : m_id(id), m_idClient(idClient), m_qte(qte), m_priorite(priorite),
      ...
      m_dateLivraison(dateLivraison), m_prixTotal(prixTotal)
{
}
...
bool Commande::ajouter()
{
    ...
    query.prepare("INSERT INTO COMMANDE (ID_COMMANDE, ID_CLIENT, QTE, PRIORITE, STATUT, DATE_COMMANDE, DATE_LIVRAISON, PRIX_TOTAL) "
                  "VALUES (:id, :client, :qte, :prio, :stat, :dcmd, :dliv, :prix)");
...
bool Commande::modifier()
{
    ...
    query.prepare("UPDATE COMMANDE SET ID_CLIENT=:client, QTE=:qte, PRIORITE=:prio, STATUT=:stat, "
                  "DATE_COMMANDE=:dcmd, DATE_LIVRAISON=:dliv, PRIX_TOTAL=:prix "
