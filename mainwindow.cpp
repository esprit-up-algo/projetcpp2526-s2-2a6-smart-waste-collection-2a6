...
QPixmap renderProduitImage(const QPixmap &src, const QSize &target)
{
    ...
    QImage scaled = base.scaled(target, Qt::KeepAspectRatioByExpanding, Qt::SmoothTransformation);
...
void applyProduitImageLabel(QLabel *lbl, const QString &imagePath, const QString &placeholder)
{
    ...
    int fixedHeight = lbl->property("prodImageHeight").toInt();
    if (fixedHeight <= 0) fixedHeight = 260;
    lbl->setMinimumSize(220, fixedHeight);
    lbl->setMaximumHeight(fixedHeight);
    ...
    if (!imagePath.isEmpty() && QFile::exists(imagePath)) {
        ...
        if (!pix.isNull()) {
            const QSize target = produitImageTargetSize(lbl);
            lbl->setPixmap(renderProduitImage(pix, target));
            lbl->setText("");
...
=======
    // ===== Initialize Floating AI Button and Email Manager =====
...
=======
        QStringList headers = { "Matricule", "Nom", "Email", "Type Contrat", "Date Expiration", "Paiement", "Actions" };
>>>>>>> Stashed changes
...

{
    ...
    QTableWidgetItem* status = new QTableWidgetItem(QString::fromUtf8("PR\xC3\x89SENT"));
...

{
    ...
    if (existing) {
        ...
        auto *welcomeMsg = new QLabel(QString::fromUtf8("Nous optimisons la collecte des d\xC3\xA9chets urbains gr\xC3\xA2ce \xC3\xA0 l'intelligence artificielle (IA) et aux objets connect\xC3\xA9s (IoT)."));
        ...
        QLabel *lblTitle2 = new QLabel(QString::fromUtf8("MARGE B\xC3\x89N\xC3\x89FICIAIRE NETTE"));
        ...
        QLabel *lblDesc2 = new QLabel(QString::fromUtf8("Pourcentage de profit r\xC3\xA9el apr\xC3\xA8s co\xC3\xBBts de production et salaires."));
        ...
        input->setPlaceholderText(QString::fromUtf8("\xC3\x89crire un message..."));
            ...
            if (sq.exec()) {
                while (sq.next()) {
                    ...
                    QString txt   = QString::fromUtf8(
                        "\u26a0\ufe0f Stock Critique \u2013 %1 (R\xC3\xa9f: %2) : %3 unit\xC3\xa9s restantes (Seuil: %4)")
        ...
        // Expiry alerts
        {
            ...
            if (sq.exec()) {
                while (sq.next()) {
                    ...
                    QString txt = QString::fromUtf8(
                        "\U0001f6d1 \xC3\x89ch\xC3\xa9ance \u2013 %1 (R\xC3\xa9f: %2) : %3")
                        .arg(nom, ref, status);
                    QListWidgetItem *item = new QListWidgetItem(txt);
...
=======
    ...
    if (auto *lblAdd = root->findChild<QLabel*>("prod_lblImgPreview_Add")) {
        lblAdd->setProperty("prodImageHeight", 250);
    ...
    if (auto *lblMod = root->findChild<QLabel*>("prod_imgBin")) {
        lblMod->setProperty("prodImageHeight", 200);
...
>>>>>>> Stashed changes
        ...
        QTimer::singleShot(200, this, [this, root]() {
            ...
            QPushButton *btnPrev = new QPushButton(QString::fromUtf8("\xE2\x97\x80 Pr\xC3\xA9c\xC3\xA9dent"));
...
=======
    ...
    QList<Proto> data = {
        {"WG-101", "Bac Intelligent 120L",   QString::fromUtf8("Ext\xC3\xA9rieur"), "120 L",  "850",  "45", "Actif",       "40", "120", "A1"},
        {"WG-102", QString::fromUtf8("Bac Connect\xC3\xA9 240L"), "Capteur",   "240 L",  "1200", "25", "Actif",       "25",  "40", "B3"},
        {"WG-103", QString::fromUtf8("Colonne Enterr\xC3\xA9e 3 FLUX"), "Industriel","3 Flux", "3400",  "0", "Maintenance",  "0", "100", "C2"},
    ...
    for (int i = 0; i < data.size(); ++i) {
        ...
        setCell(i, 2, data[i].cap);
        setCell(i, 3, data[i].price);
        setCell(i, 4, data[i].aisle);
        setCell(i, 5, data[i].etat);
        // Store hidden capacity + aisle data in roles
        t->item(i, 0)->setData(PROD_ROLE_CUR, data[i].cur);
        t->item(i, 0)->setData(PROD_ROLE_MAX, data[i].max);
        t->item(i, 0)->setData(PROD_ROLE_AISLE, data[i].aisle);
        // Backward-compatible roles
...
=======
        QStringList headers = { "Reference", "Date", "Technicien", QString::fromUtf8("Co\xC3\xBBt"), QString::fromUtf8("Dur\xC3\xA9e"), QString::fromUtf8("Priorit\xC3\xA9"), "Actions" };
>>>>>>> Stashed changes
...
void MainWindow::setupStockTableData()
{
    ...
    // 2. Choix de la source de données (Base de données OU valeurs de test)
    if (model->rowCount() == 0) {
        ...
        for (int i = 0; i < items.size(); ++i) {
            int stockQty = items[i].stock.toInt();
            double prix = items[i].prix.split(" ")[0].toDouble();
            // idMp négatif pour les différencier des vraies données
            insertStockRow(i, -(i+1), items[i].ref, items[i].nom, stockQty, 5, items[i].seuil, prix, items[i].fournisseur, "", QDate::currentDate(), QDate::currentDate());
        }
    } else {
...
>>>>>>> Stashed changes
    ...
    QMessageBox::information(this, QString::fromUtf8("Succ\xC3\xA8s"), QString::fromUtf8("Client ajout\xC3\xA9 avec succ\xC3\xA8s."));
...
=======
    // Find client ID by matricule
    int clientId = -1;
    Client tempClient;
    if (!tempClient.findIdByMatricule(ui->input_matricule_modifier->text(), clientId)) {
        QMessageBox::warning(this, "Erreur", "Client introuvable: " + tempClient.lastError());
        return;
    }
...
>>>>>>> Stashed changes
    ...
    QMessageBox::information(this, QString::fromUtf8("Succ\xC3\xA8s"), QString::fromUtf8("Informations du client modifi\xC3\xA9es avec succ\xC3\xA8s."));
...
=======
        ui->tableWidget_Client->item(row, 3)->setText(type_contrat);

        ui->tableWidget_Client->item(row, 4)->setText(date_expiration);
>>>>>>> Stashed changes
...
=======
        QString dateStr = ui->tableWidget_Client->item(r, 4)->text();
...
=======
            // Get matricule from row
            QTableWidgetItem* matriculeItem = ui->tableWidget_Client->item(r, 0);
            if (!matriculeItem) return;

            // Find client ID
            int clientId = -1;
            Client tempClient;
            if (!tempClient.findIdByMatricule(matriculeItem->text(), clientId)) {
...
>>>>>>> Stashed changes
            ...
            if (!tempClient.supprimer(clientId)) {
                QMessageBox::critical(this, "Erreur", "Impossible de supprimer le client de la base de donnees");
                return;
            }
            ...
            QSqlDatabase db = QSqlDatabase::database();
            if (!db.commit()) {
                qDebug() << "Erreur commit apres supprimer client:" << db.lastError().text();
            }
...
void MainWindow::on_btnPdf_Cmd_clicked()
{
    ...
    QStringList headers = {"ID", QString::fromUtf8("Quantit\xC3\xA9"), QString::fromUtf8("Priorit\xC3\xA9"), "Status", "Date", "Prix Total"};
    int x = xStart;
    int colWidth = 1300;
...
void MainWindow::refreshCommandes()
{
    ...
    for(int i=0; i<model->rowCount(); ++i) {
        ...
        idItem->setData(Qt::UserRole, model->data(model->index(i, 8)).toInt());
        
        ui->tableProduits_2->setItem(row, 0, idItem); // ID
        ...
        ui->tableProduits_2->setItem(row, 4, new QTableWidgetItem("---")); // Adresse (placeholder)
        ui->tableProduits_2->setItem(row, 5, new QTableWidgetItem(model->data(model->index(i, 5)).toDate().toString("yyyy-MM-dd"))); // Date Cmd
        ui->tableProduits_2->setItem(row, 6, new QTableWidgetItem("---")); // Stock (placeholder)
...
void MainWindow::on_btnSave_Mod_3_clicked()
{
    ...
    // Simple date construction (assuming combos are populated or default)
    QDate dCmd = QDate::currentDate(); 
    QDate dLiv = QDate::currentDate().addDays(7);
...
void MainWindow::on_btnSave_Mod1_clicked()
{
    ...
    // Preserve existing dates (or add DateEdit widgets to modify form if needed)
    QDate dCmd = QDate::fromString(ui->tableProduits_2->item(currentCmdRow, 5)->text(), "yyyy-MM-dd");
    QDate dLiv = QDate::fromString(ui->tableProduits_2->item(currentCmdRow, 7)->text(), "yyyy-MM-dd");
...
void MainWindow::loadProduitToModificationForm(int row)
{
    ...
    if (ui->prod_imgBin) {
        ui->prod_imgBin->setProperty("prodImageHeight", 200);
...
void MainWindow::on_prod_btnUpload_Add_clicked()
{
    ...
    if (!fileName.isEmpty()) {
        ...
        if (ui->prod_lblImgPreview_Add) {
            ui->prod_lblImgPreview_Add->setProperty("prodImageHeight", 250);
...
void MainWindow::showClientStats() {
    ...
    // ===== PROFESSIONAL ECOSCORE SYSTEM =====
    // Calculate real EcoScore based on waste sorting compliance metrics
    
    // 1. EcoScore Ranking Bar Chart (by waste sorting rate)
    query.exec(
        "SELECT "
        "c.ID_CLIENT, c.NOM, c.TYPE_CONTRAT, c.STATUT_PAIEMENT, "
        "COUNT(i.ID_INTER) as maintenance_count, "
        "NVL(SUM(CASE WHEN i.STATUT='Terminée' THEN 1 ELSE 0 END), 0) as completed_count "
        "FROM CLIENT c "
        "LEFT JOIN INTERVENTION i ON c.ID_CLIENT = i.ID_BAC "
        "GROUP BY c.ID_CLIENT, c.NOM, c.TYPE_CONTRAT, c.STATUT_PAIEMENT "
        "ORDER BY c.NOM"
    );
    
    QBarSet *ecoScoreSet = new QBarSet("EcoScore Taux de Tri");
    ...
    while (query.next()) {
        ...
        int maintCount = query.value(4).toInt();
        int completedCount = query.value(5).toInt();
        
        // EcoScore Calculation based on:
        // 1. Contract type (90=Annuel, 70=Trimestriel, 50=Mensuel)
        // 2. Payment status (Payé=full, En Retard=0.7)
        // 3. Maintenance compliance (completed/total = bonus up to 20 points)
        ...
        double paymentMultiplier = statut.contains("Retard", Qt::CaseInsensitive) ? 0.7 : 1.0;
        
        double complianceBonus = 0;
        if (maintCount > 0) {
            double complianceRate = (double)completedCount / maintCount;
            complianceBonus = complianceRate * 20; // Up to 20 points bonus
        }
        
        double ecoScore = (baseScore * paymentMultiplier) + complianceBonus;
        ecoScore = qMin(100.0, ecoScore); // Cap at 100
        ecoScore = qMax(0.0, ecoScore);   // Floor at 0
        
        *ecoScoreSet << ecoScore;
        ecoScores.append(ecoScore);
        clientNames << (nom.length() > 12 ? nom.left(12) + ".." : nom);
    }
    ...
    barChart->setTitleBrush(QColor("#27ae60"));
    barChart->setAnimationOptions(QChart::SeriesAnimations);
    barChart->setBackgroundBrush(QColor("#f8fafc"));
    ...
    axisY->setTitleText("Score (0-100)");
    axisY->setTitleFont(QFont("Segoe UI", 10, QFont::Bold));
    axisY->setTitleBrush(QColor("#0f2b4c"));
    axisY->setLabelFormat("%d");
    axisY->setLabelsColor(QColor("#64748b"));
    barChart->addAxis(axisY, Qt::AlignLeft);
    barSeries->attachAxis(axisY);
    
    QChartView *chartEcoScoreBar = findChild<QChartView*>("chartEcoScoreBar");
    if (chartEcoScoreBar) {
        if (chartEcoScoreBar->chart()) delete chartEcoScoreBar->chart();
        chartEcoScoreBar->setChart(barChart);
        chartEcoScoreBar->setRenderHint(QPainter::Antialiasing);
    }
    
    // 2. EcoScore by Rating Grade (A, B, C, D)
    int gradeA = 0, gradeB = 0, gradeC = 0, gradeD = 0;
    for (double score : ecoScores) {
        if (score >= 85) gradeA++;
...
void MainWindow::on_btnSave_add_clicked()
{
    ...
    int seuil = (ui->inputSeuil_add && !ui->inputSeuil_add->text().isEmpty()) ? ui->inputSeuil_add->text().toInt() : 0;
...
void MainWindow::on_btnSave_mod_clicked()
{
    ...
    int seuil = (ui->inputSeuil_mod && !ui->inputSeuil_mod->text().isEmpty()) ? ui->inputSeuil_mod->text().toInt() : 0;
...
void MainWindow::checkAndNotifyExpiringContracts()
{
    ...
    QDate notificationThreshold = today.addDays(7); // Notify 7 days before expiration
...
void MainWindow::sendContractExpirationEmail(const QString &clientEmail, const QString &clientName, const QString &expirationDate)
{
    ...
    // Enhanced logging for email notification
    qDebug() << "═══════════════════════════════════════════════════════════";
    qDebug() << "📧 EMAIL NOTIFICATION SYSTEM";
    qDebug() << "═══════════════════════════════════════════════════════════";
    qDebug() << "Status:" << "✓ QUEUED FOR SENDING";
    qDebug() << "To:" << clientEmail;
    qDebug() << "Client:" << clientName;
    qDebug() << "Subject:" << subject;
    qDebug() << "Expiration Date:" << expirationDate;
    qDebug() << "Days Until Expiration:" << daysLeft;
    qDebug() << "Sent:" << QString("%1 %2").arg(currentDate, currentTime);
    qDebug() << "HTML Content Length:" << htmlBody.length() << "bytes";
    qDebug() << "═══════════════════════════════════════════════════════════";

    // Show professional notification toast in the UI
    if (ui->stackedWidget_Client) {
        QString notificationMsg = QString(
            "📬 EMAIL NOTIFICATION\n\n"
            "Destinataire: %1\n"
            "Client: %2\n"
            "Date d'expiration: %3\n"
            "Jours restants: %4\n\n"
            "L'email de notification a été mis en queue pour envoi."
        ).arg(clientEmail, clientName, expirationDate, QString::number(daysLeft));

        // Show information dialog instead of just message box for better visibility
        ...
        msgBox.setStyleSheet(
            ...
            "  background: qlineargradient(x1:0,y1:0,x2:1,y2:1, stop:0 #0f2b4c, stop:1 #2a5298); "
            "  color: white; border: none; border-radius: 8px; padding: 8px 20px; font-weight: 700; "
            "} "
            "QMessageBox QPushButton:hover { background: #1e40af; }");
        msgBox.exec();
    }
}
