...
void MainWindow::setupMaintenanceModule() {
    ...
    if (auto* table = maintenanceTable()) {
        ...
        table->setColumnHidden(7, true); // hidden column for ID_INTER
...
void MainWindow::on_btnSave_Mod_clicked() {
    ...
    int idInter = 0;
    QTableWidgetItem *idItem = table->item(currentMaintRow, 7);
...
void MainWindow::handleMaintEditClicked() {
    ...
    ui->editTechMod->setText(table->item(row, 2)->text()); // Statut in "Technicien" field
...
void MainWindow::handleMaintDeleteClicked() {
    ...
    if (QMessageBox::question(this, "Supprimer", "Voulez-vous vraiment supprimer cette intervention ?", QMessageBox::Yes|QMessageBox::No) == QMessageBox::Yes) {
        ...
        int idInter = 0;
        auto* table = maintenanceTable();
        QTableWidgetItem *idItem = table->item(row, 7);
        if (idItem) idInter = idItem->text().toInt();
...
void MainWindow::refreshInterventions() {
    ...
    for (int i = 0; i < model->rowCount(); ++i) {
        ...
        table->setItem(row, 0, new QTableWidgetItem(model->data(model->index(i, 1)).toString())); // Reference
        table->setItem(row, 1, new QTableWidgetItem(model->data(model->index(i, 2)).toDate().toString("yyyy-MM-dd"))); // Date
        table->setItem(row, 2, new QTableWidgetItem(model->data(model->index(i, 5)).toString())); // Statut
        table->setItem(row, 3, new QTableWidgetItem(QString::number(cout, 'f', 2) + " DT"));      // Coût
        table->setItem(row, 4, new QTableWidgetItem(dureeStr));                                     // Durée
        table->setItem(row, 5, new QTableWidgetItem(model->data(model->index(i, 7)).toString())); // Priorité
        table->setItem(row, 7, new QTableWidgetItem(QString::number(idInter)));                    // Hidden ID
